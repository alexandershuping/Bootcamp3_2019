...
describe('Listings CRUD tests', function() {
  ...
  it('should be able to delete the listing with coordinates', function(done) {
	...
	it('should not break when given an invalid address', function(done) {
		bad_listing = {
			code: 'AAA1111',
			name: 'MALFORMED LOCATION',
			address: 'SoM3BaD@DdReS$'
		}

		agent.post('/api/listings').send(bad_listing)
			.expect(200)
			.end(function(err, res){
				should.not.exist(err)
				should.exist(res.body._id)
				res.body.code.should.equal('AAA1111')
				res.body.name.should.equal('MALFORMED LOCATION')
				res.body.address.should.equal('SoM3BaD@DdReS$')
				should.not.exist(res.body.coordinates)
				bad_id = res.body._id
				done()
			})
	})
  ...
  after(function(done) {
    if(id) {
      ...
      Listing.deleteOne({_id: id2}, function(err){
		...
		if(bad_id){
			Listing.deleteOne({_id: bad_id}, function(err){
				if(err) throw err
			})
		}
